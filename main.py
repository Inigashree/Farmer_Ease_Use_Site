...
def transportation():
    ...
    if customer_id is None:
        flash('Please log in to view transportation services.')
        return redirect(url_for('customer_login1'))
    customers_using_transport = Customer.query.filter_by(using_transport=True).all()
    return render_template('transportation.html', customers=customers_using_transport)
@app.route('/customer_login1', methods=['GET'])
def customer_login1():
    farmer_id = session.get('farmer_id')  # Ensure only farmers access this
    if farmer_id is None:
        flash('Please log in as a farmer to view transportation requests.')
        return redirect(url_for('farmer_login'))
    
    # Fetch customers who have registered for transportation
    customers = Customer.query.filter_by(registered_for_transport=True).all()
    return render_template('customer_login1.html', customers=customers)
# Logout
